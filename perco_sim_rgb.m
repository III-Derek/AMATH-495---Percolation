...
% Define colors for 0s (red, green, yellow) and 1s (blue)
colors0 = [1 0 0; 0 1 0; 1 1 0]; % red, green, yellow
color1 = [0 0 1]; % blue
...
for p = prob_range
    for iteration = 1:1
        ...
        % Assign colors based on matrix values
        for i = 1:L
            for j = 1:L
                if occupancyGrid(i, j) == 0
                    % Assign random color for 0s
                    randColorIndex = randi(size(colors0, 1));
                    rgbImage(i, j, :) = colors0(randColorIndex, :);
                else
                    % Assign blue color for 1s
                    rgbImage(i, j, :) = color1;
                end
            end
        end
        % saving the images
        % make a folder 'perco_img_rgb' on desktop
        filePath = '/Users/chengxinye/Desktop/perco_img_rgb';
        image_name = [num2str(p), '_', num2str(max_cluster)];
        fullFilePath = fullfile(filePath, [image_name, '.png']);
        imwrite(rgbImage, fullFilePath, "png");
